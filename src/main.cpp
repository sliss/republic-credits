...
#if defined(NDEBUG)
# error "RepublicCredits cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x274dd4976ec4182e4bac161cf6209b7575398d469e381125cc32965f252e38b5k");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // RepublicCredits: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "RepublicCredits Signed Message:\n";
...
{
    // RepublicCredits: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 660000 blocks, which will occur approximately every pi years
    nSubsidy >>= (nHeight / 660000); // RepublicCredits: 660 blocks in ~pi years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // RepublicCredits: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // RepublicCredits: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // RepublicCredits: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // RepublicCredits: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x53a3a87a9c18c6f167ba5d53baeeea49d4d0897b0d215c63c6b531bbd83f462f");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "NY Times 17/Feb/2018 Robert Mueller indicts 13 Russians for election interference";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d35873b4f56c3c687b3f4699d2baa29e544b75d2a5e7e2f28e43094c0fdf41b17bc35a5e439d23bfc8756f56090543d8d28928a7cb7dace3fa9cfb1eb7084045") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1519086378;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084945180;

        if (fTestNet)
        {
            block.nTime    = 1519086361;
            block.nNonce   = 385594677;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf07eb549fae81a9e1f13f6264dcc5e40009a80fb83390526f82d0340f00b0d23"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb0, 0x55, 0x9b, 0xdd }; // RepublicCredits: change each
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("RepublicCreditsMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("RepublicCreditsMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static RepublicCreditsMiner(CWallet *pwallet)
{
    printf("RepublicCreditsMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("republiccredits-miner");
    ...
    try { loop {
        ...
        printf("Running RepublicCreditsMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("RepublicCreditsMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&RepublicCreditsMiner, pwallet));
}
